...
def calculate(expression,val_type):
    ...
    if len(expression)==1:return expression
        ...
        if len(up_bakets_positions)>0:
            if(expression[0:up_bakets_positions[0]])!='' and expression[down_brackets_positions[-1]+1:]!='':
            ...
            if(expression[0:up_bakets_positions[0]])=='' and expression[down_brackets_positions[-1]+1:]!='': 
                return calculate(calculate(expression[up_bakets_positions[0]+1:down_brackets_positions[-1]],val_type)+expression[down_brackets_positions[-1]+1:],val_type)   
            if(expression[0:up_bakets_positions[0]])!='' and expression[down_brackets_positions[-1]+1:]=='': 
                return calculate(expression[0:up_bakets_positions[0]]+calculate(expression[up_bakets_positions[0]+1:down_brackets_positions[-1]],val_type),val_type)
            if(expression[0:up_bakets_positions[0]])=='' and expression[down_brackets_positions[-1]+1:]=='':
                return calculate(expression[up_bakets_positions[0]+1:down_brackets_positions[-1]],val_type)
        else:
